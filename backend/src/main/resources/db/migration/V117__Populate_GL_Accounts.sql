...
('CBA_CALL_DEPOSITS', 'CBA Call Deposits', 'ASSET', 'AGGREGATION',
  JSON_OBJECT('table','accounts','field','balance','where','account_type = ''SAVINGS'''), 20),

('CBA_CURRENT', 'CBA Current Account', 'ASSET', 'AGGREGATION',
  JSON_OBJECT('table','accounts','field','balance','where','account_type = ''CURRENT'''), 21),
...
('MEMBER_DEPOSITS', 'Member Deposits', 'LIABILITY', 'AGGREGATION',
  JSON_OBJECT('table','accounts','field','balance','where','account_type = ''SAVINGS'''), 40),

('MEMBER_SHARES', 'Member Shares', 'LIABILITY', 'AGGREGATION',
  JSON_OBJECT('table','accounts','field','balance','where','account_type = ''SHARES'''), 41),
...
-- REVENUE ACCOUNTS
INSERT INTO gl_accounts (code, name, account_type, balance_calculation_type, calculation_config, display_order) VALUES
('LOAN_INTEREST', 'Loan Interest Income', 'REVENUE', 'AGGREGATION',
  JSON_OBJECT('table','transactions','field','amount','transactionType','INTEREST'), 70),
...
('ENTRANCE_FEES', 'Entrance Fees', 'REVENUE', 'AGGREGATION',
  JSON_OBJECT('table','transactions','field','amount','transactionType','ENTRANCE_FEE'), 72),
...
('BANK_CHARGES', 'Bank Charges', 'EXPENSE', 'AGGREGATION',
  JSON_OBJECT('table','transactions','field','amount','transactionType','BANK_CHARGE'), 87),
